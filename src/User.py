"""
Automatically translated from C++ to Python

Original file: src/User.cpp
Module: User

This code was generated by an AI-powered code translation system.
Please review and test before using in production.
"""

class User:
    def __init__(self, user_id, name, email, max_books):
        self.user_id = user_id
        self.name = name
        self.email = email
        self.max_books_allowed = max_books
        self.borrowed_books = []
        print(f"User registered: {self.name}")

    def __del__(self):
        print(f"User removed: {self.name}")

    def get_user_id(self):
        return self.user_id

    def get_name(self):
        return self.name

    def get_email(self):
        return self.email

    def get_borrowed_book_count(self):
        return len(self.borrowed_books)

    def can_borrow_more(self):
        return len(self.borrowed_books) < self.max_books_allowed

    def borrow_book(self, isbn):
        if not self.can_borrow_more():
            print("Cannot borrow more books. Limit reached!")
            return False

        if self.has_borrowed(isbn):
            print("Book already borrowed!")
            return False

        self.borrowed_books.append(isbn)
        print(f"{self.name} borrowed book: {isbn}")
        return True

    def return_book(self, isbn):
        if isbn in self.borrowed_books:
            self.borrowed_books.remove(isbn)
            print(f"{self.name} returned book: {isbn}")
            return True

        print("Book not found in borrowed list!")
        return False

    def has_borrowed(self, isbn):
        return isbn in self.borrowed_books

    def display_info(self):
        print(f"User ID: {self.user_id}")
        print(f"Name: {self.name}")
        print(f"Email: {self.email}")
        print(f"Books borrowed: {len(self.borrowed_books)}/{self.max_books_allowed}")

        if self.borrowed_books:
            print("Borrowed ISBNs: ", end="")
            print(" ".join(self.borrowed_books))